...
mod tests {
    ...
    fn test_base(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "debug-comm-receive-log-{name}-{}",
            LOG_SEQUENCE.fetch_add(1, Ordering::Relaxed)
        ))
    }

    #[test]
    fn reads_recent_records_old_to_new() {
        let base = test_base("recent");
        for index in 0..120 {
            let record = ReceiveLogRecord {
                id: format!("{index:013}-0000000001"),
                received_at: index,
                workflow_id: "workflow".to_string(),
                step_id: "output".to_string(),
                source_step_id: "source".to_string(),
                byte_len: 1,
                msg: Value::String(index.to_string()),
            };
            append_record(&base, &record).unwrap();
        }

        let records = read_records(&base, "workflow", "output", None, None).unwrap();

        assert_eq!(records.len(), 100);
        assert_eq!(records.first().unwrap().id, "0000000000020-0000000001");
        assert_eq!(records.last().unwrap().id, "0000000000119-0000000001");

        let _ = fs::remove_dir_all(base);
    }

    #[test]
    fn reads_records_before_cursor() {
        let base = test_base("before");
        for index in 0..10 {
            let record = ReceiveLogRecord {
                id: format!("{index:013}-0000000001"),
                received_at: index,
    ...
    fn clears_records() {
        ...
        clear_records(&base, "workflow", "output").unwrap();
        let records = read_records(&base, "workflow", "output", None, None).unwrap();
